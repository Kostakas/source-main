    ...
    public class PartyMemberKickPacketProcessor : IGamePacketProcessor
    {
        ...
        public async Task Process(GameClient client, byte[] packetData)
        {
            ...
            party.RemoveMember(party[targetName].Key);
             ...
             if (party.Members.Count < 2) // TESTE MEMBRO REMOVIDO
               {             
                _logger.Information($"[ TESTE ] MEMBRO REMOVIDO: {targetName}");
            ...
            {
                _logger.Information($"[ TESTE 2] MEMBRO REMOVIDO: {targetName}");
